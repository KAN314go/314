...
async function main() {
    ...
    let json_config = {
        ...
        "homeLogo": "./logo500x200-1.png",
        "spider": "./pg.jar?md5=7633f8ea346c082b7aa163be58aed023",
        'sites': [],
        'parses': parses,
        'flags': [
            'imgo',
    ...
    js_files.forEach((it, index) => {
        ...
        if (rname.includes('我的哔哩[官]')) {
            extras = [
                '?type=url&params=./小学教育.json@哔哩教育[官]',
                '?type=url&params=./哔哩大全.json@哔哩大全[官]',
            ];
        } else if (rname.includes('采集之王')) {
            extras = [
                '?type=url&params=./采集静态.json$1@采王道长[合]',
                '?type=url&params=./采集[zy]静态.json$1@采王zy[密]',
                '?type=url&params=./采集[密]静态.json@采王成人[密]',
            ];
        } else if (rname.includes('直播转点播')) {
            extras = [
                '?type=url&params=./live2cms.json',
            ];
        }
